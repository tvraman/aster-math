...
(defmacro without-reading-style ((style-name)  &body body)
  ...
  `(let ((style-flag (find ',style-name (current-reading-style ))))
     (unwind-protect
          (progn
...
(defmacro without-reading-rule ((object-name rule-name) &body body)
  "Execute body with rule rule-name for object object-name deactivated. "
  `(let ((rule-flag
           (eq ',rule-name (active-rule  (make-instance ',object-name)))))
     (unwind-protect
          (progn
...
(defmacro with-reading-rule ((object-name rule-name) &body body)
  ...
  `(let ((rule-p (eq ',rule-name (active-rule  (make-instance ',object-name)))))
     (unwind-protect
          (progn
            (unless  rule-p (activate-rule ',object-name))
            ,@body)
       (unless  rule-p (deaactivate-rule ',object-name ',rule-name)))))
...
(defmacro with-added-reading-style ((style-name) &body body)
  "Execute body with style style-name added at the end."
  `(let ((style-flag (find ',style-name (current-reading-style ))))
     (unwind-protect
          (progn
